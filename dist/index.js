...
function deriveSiblingFfprobePath(ffmpegPath) {
...
async function getSavedCookiesPath() {
    ...
    if (await sigma.fs.storage.exists(MANAGED_COOKIES_RELATIVE_PATH)) {
        const legacyPath = await sigma.storage.get(COOKIES_LEGACY_SOURCE_PATH_STORAGE_KEY);
        if (legacyPath) {
            await sigma.storage.remove(COOKIES_LEGACY_SOURCE_PATH_STORAGE_KEY);
        }
        return managedPath;
    }
    const storedPath = await sigma.storage.get(COOKIES_LEGACY_SOURCE_PATH_STORAGE_KEY);
    if (!storedPath) {
        return null;
    }
...
async function ensureDenoInstalled() {
    if (cachedDenoBinaryPath)
        return cachedDenoBinaryPath;
    try {
        ...
        if (sigma.runtime.isExtensionInstallCancelledError(error))
            throw error;
...
function getVideoQualityOptions(translate) {
    return [
...
function getTwitchQualityOptions(translate) {
    return [
...
function getAudioQualityOptions(translate) {
    return [
        { value: 'best', label: translate('bestAvailable') },
...
function getDownloadModes(translate) {
    return [
...
function buildFormatSelector(options) {
    ...
    if (platform === 'twitch-live' || platform === 'twitch-vod') {
        if (twitchQuality === 'best')
            return 'best';
        if (twitchQuality === 'audio_only')
            return 'audio_only';
...
function shouldRetryWithBrowserCookies(options, outputText) {
    ...
    return (lowerOutput.includes('no title found in player responses')
...
function buildPreviewContent(url, videoInfo, previewState) {
    ...
    if (videoInfo && videoInfo.thumbnail) {
        baseContent.push(sigma.ui.text(t('supportsYoutubeTwitch')), sigma.ui.previewCard({
            thumbnail: videoInfo.thumbnail,
            title: videoInfo.title || t('untitled'),
            subtitle: videoInfo.subtitle || '',
        }), sigma.ui.separator(), sigma.ui.select({
            id: 'mode',
...
async function ensureYtDlpWrapper(ytDlpBinaryPath, pathPrependDirs) {
    if (cachedWrapperPath)
...
async function runYtDlp(binaryPath, options) {
    ...
    const handleProgressLine = (payload) => {
        ...
        if (line.startsWith('PROG\t') || line.includes('[download]')) {
            ...
            if (info.percent !== null) {
                ...
                if (info.size !== null)
                    lastDownloadState.size = info.size;
                if (info.speed !== null)
                    lastDownloadState.speed = info.speed;
                if (info.eta !== null)
                    lastDownloadState.eta = info.eta;
                throttledUpdate({
    ...
    async function runYtDlpAttempt(attemptArgs, attemptLabel) {
        ...
        const commandTask = await sigma.shell.runWithProgress(effectiveBinaryPath, attemptArgs, handleProgressLine);
...
async function handleDownloadCommand(prefilledUrl) {
    ...
    if (progressResult.needsCookieSetup) {
        const cookieSetupResult = await showCookieSetupModal();
        if (!cookieSetupResult || cookieSetupResult.action !== 'imported')
            return;
