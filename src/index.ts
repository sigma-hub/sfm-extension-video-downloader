import type { ExtensionActivationContext, ProgressReport, UIElement } from '@sigma-file-manager/api';
import type {
  CookieSetupResult,
  DownloadModalResult,
...
function getDirectoryFromPath(binaryPath: string | null): string | null {
...
function isPathWithinPath(pathValue: string, rootPath: string): boolean {
...
async function importCookiesFile(sourcePath: string): Promise<string> {
  await sigma.fs.storage.importFile(sourcePath, MANAGED_COOKIES_RELATIVE_PATH);
  await sigma.storage.remove(COOKIES_LEGACY_SOURCE_PATH_STORAGE_KEY);
  return getManagedCookiesFilePath();
}
...
async function getSavedCookiesPath() {
  ...
  if (await sigma.fs.storage.exists(MANAGED_COOKIES_RELATIVE_PATH)) {
    const legacyPath = await sigma.storage.get<string>(COOKIES_LEGACY_SOURCE_PATH_STORAGE_KEY);
    if (legacyPath) {
      await sigma.storage.remove(COOKIES_LEGACY_SOURCE_PATH_STORAGE_KEY);
    }
    return managedPath;
  ...
  const storedPath = await sigma.storage.get<string>(COOKIES_LEGACY_SOURCE_PATH_STORAGE_KEY);
  if (!storedPath) {
    return null;
  }
  ...
  if (cachedExtensionStoragePath && isPathWithinPath(storedPath, cachedExtensionStoragePath)) {
    try {
      if (await sigma.fs.exists(storedPath)) {
        return storedPath;
      }
    } catch {
    }
...
async function ensureDenoInstalled() {
  ...
  try {
    const denoPath = await sigma.binary.getPath(DENO_BINARY_ID);
    if (!denoPath) {
      return null;
    }
    ...
    console.error('[Video Downloader] Failed to resolve Deno path:', error);
    return null;
  }
}
...
async function renamePartFilesToTs(directory: string): Promise<void> {
  try {
...
function parseDownloadInfo(line: string): DownloadProgressState {
  const delim = '\t';
...
function parseFfmpegProgress(line: string): FfmpegProgressState | null {
...
function formatStreamProgressMessage(info: FfmpegProgressState): string {
  const parts = [];
  ...
  return parts.length > 0 ? parts.join(' • ') : (t('recordingStream') || 'Recording stream...');
}

function formatStatusMessage(line: string): string {
  const message = line.replace(/^\[[^\]]+\]\s*/, '').trim();
  const maxLength = 60;
  return message.length > maxLength ? message.substring(0, maxLength) + '...' : message;
}

function normalizePreviewErrorMessage(rawError: unknown): string {
  if (!rawError) return '';
...
function isYouTubePreviewAuthError(rawError: string): boolean {
...
function getVideoQualityOptions(translate: TranslateFn) {
  return [
    { value: 'best', label: translate('bestAvailable') },
    { value: '1080', label: translate('quality1080') },
    { value: '720', label: translate('quality720') },
    { value: '480', label: translate('quality480') },
    { value: '360', label: translate('quality360') }
  ];
}

function getTwitchQualityOptions(translate: TranslateFn) {
  return [
    { value: 'best', label: translate('sourceBest') },
    { value: '1080p60', label: translate('quality1080p60') },
    { value: '1080p', label: translate('quality1080p') },
    { value: '720p60', label: translate('quality720p60') },
    { value: '720p', label: translate('quality720p') },
    { value: '480p', label: translate('quality480p') },
    { value: '360p', label: translate('quality360p') },
    { value: 'audio_only', label: translate('audioOnly') }
  ];
}

function getAudioQualityOptions(translate: TranslateFn) {
  return [
    { value: 'best', label: translate('bestAvailable') },
    { value: 'medium', label: translate('mediumQuality') },
    { value: 'low', label: translate('lowQuality') }
  ];
}

function getDownloadModes(translate: TranslateFn) {
  return [
    { value: 'video-audio', label: translate('videoAndAudio') },
    { value: 'video-only', label: translate('videoOnly') },
    { value: 'audio-only', label: translate('audioOnly') }
  ];
}

function detectPlatform(url: string): PlatformKind {
...
async function ensureBinaryInstalled() {
  const binaryPath = await sigma.binary.getPath(YTDLP_BINARY_ID);
  if (!binaryPath) {
    throw new Error('yt-dlp binary is unavailable');
  }
...
function buildFormatSelector(options: YtDlpBuildOptions): string | null {
...
function shouldRetryWithBrowserCookies(
  options: Pick<YtDlpBuildOptions, 'platform'>,
  outputText: string,
): boolean {
...
async function showCookieSetupModal(): Promise<CookieSetupResult> {
  const existingCookiesPath = await getSavedCookiesPath();
  const hasSavedCookies = Boolean(existingCookiesPath);

  return new Promise<CookieSetupResult>((resolve) => {
    const modal = sigma.ui.createModal({
      ...
      buttons: [
        ...
        ...(hasSavedCookies ? [{ id: 'clear', label: t('clearStoredCookies'), variant: 'danger' as const }] : []),
      ],
    });
    ...
    modal.onSubmit(async (_values, buttonId) => {
      if (buttonId === 'select') {
        ...
        try {
          ...
          sigma.ui.showNotification({
            title: t('extensionTitle'),
            subtitle: message || t('failedToImportCookies'),
            type: 'error',
          });
          resolve(null);
        }
...
function buildPreviewContent(
  ...
  if (videoInfo && videoInfo.thumbnail) {
    ...
    if (previewState?.statusElements?.length) {
      ...
      baseContent.push(sigma.ui.text(t(previewState.statusTextKey)));
    }
...
function formatPreviewSubtitle(data: Record<string, unknown>): string | null {
  const parts: string[] = [];
  ...
  if (liveStatus === 'is_live') {
    ...
    const count = data.n_entries ?? data.playlist_count ?? entries?.length;
  ...
  if (uploader !== undefined && uploader !== null && uploader !== '') {
    parts.push(String(uploader));
  }

  return parts.join(' · ') || null;
}

function buildVideoInfoArgs(
  url: string,
  options: {
...
async function thumbnailUrlToDataUrl(thumbnailUrl: string): Promise<string> {
  try {
    ...
    console.warn('[Video Downloader] thumbnailUrlToDataUrl failed, using URL:', caughtError);
    return thumbnailUrl;
  }
}
...
): Promise<{ title: string; thumbnail: string; subtitle: string | null } | { error: string }> {
  try {
    ...
    const thumbnails = data.thumbnails as { url?: string; width?: number }[] | undefined;
    if (thumbnails && thumbnails.length > 0) {
      const withUrl = thumbnails.filter((thumb) => thumb.url);
      if (withUrl.length > 0) {
        ...
        const suitable = byWidth.find((entry) => (entry.width ?? 0) >= targetWidth);
        thumbnail = (suitable ?? byWidth[0]).url ?? null;
      }
    }
    if (!thumbnail) {
      const direct = data.thumbnail;
      thumbnail = typeof direct === 'string' ? direct : null;
    }
    if (!thumbnail) {
    ...
    return { title: String(data.title ?? ''), thumbnail: dataUrl, subtitle };
...
async function createDownloadModal(
  prefilledUrl?: string,
): Promise<DownloadModalResult | null> {
  let videoInfoValid = false;
  let debounceTimeout: ReturnType<typeof setTimeout> | null = null;
  let fetchAborted = false;

  return new Promise<DownloadModalResult | null>((resolve) => {
    const modal = sigma.ui.createModal({
    ...
    async function onUrlChange(urlValue: unknown): Promise<void> {
      ...
      debounceTimeout = setTimeout(async () => {
        ...
        if (!('error' in videoInfo) && videoInfo.thumbnail) {
          ...
          const fullContent = buildPreviewContent(urlForFetch, videoInfo as VideoInfo, null);
          if (showCookieWarning) {
            ...
            fullContent.push({
              type: 'button',
              id: 'setup-cookies',
              label: t('setupYoutubeCookiesLabel'),
              variant: 'primary' as const,
            });
            modal.setButtons([]);
          } else {
            modal.setButtons([
              { id: 'download', label: t('download'), variant: 'primary' as const, shortcut: { key: 'Enter' } },
            ]);
          }
          modal.setContent(fullContent);
          modal.updateElement('url', { value: urlForFetch });
        } else {
          const rawError = 'error' in videoInfo ? videoInfo.error : '';
    ...
    modal.onSubmit(async (values, buttonId) => {
      ...
      resolve({
        url: url.trim(),
        platform,
        mode: typeof values.mode === 'string' ? values.mode : 'video-audio',
        videoQuality: typeof values.videoQuality === 'string' ? values.videoQuality : 'best',
        audioQuality: typeof values.audioQuality === 'string' ? values.audioQuality : 'best',
        twitchQuality: typeof values.videoQuality === 'string' ? values.videoQuality : 'best',
        liveFromStart: values.liveFromStart === true,
      });
    });
...
async function runYtDlp(
...
): Promise<{ success: boolean; needsCookieSetup: boolean }> {
  ...
  let lastDownloadState: { size: string | null; speed: string | null; eta: string | null } = {
    size: null,
    speed: null,
    eta: null,
  };
  let lastUpdateTime = 0;
  let pendingUpdate: { data: ProgressReport; timeout: ReturnType<typeof setTimeout> } | null = null;
  let isLiveStream = false;
  let streamDetected = false;
  let isCancelled = false;
  let activeCancelCommand: (() => Promise<void>) | null = null;
  let cookieRetriesExhausted = false;
  const UPDATE_INTERVAL = 200;

  function throttledUpdate(update: ProgressReport): void {
  ...
  const handleProgressLine = (payload: { line?: string }) => {
  ...
  async function runYtDlpAttempt(attemptArgs: string[], attemptLabel: string) {
  ...
  if (commandResult.code !== 0 && !isCancelled) {
    ...
    if (shouldRetryWithBrowserCookies(options, fullOutput)) {
      ...
      if (savedCookiesPath && !isCancelled) {
        throttledUpdate({ subtitle: t('checkingYoutubeAccess'), description: t('tryingSavedCookies') });
      ...
      if (commandResult.code !== 0 && !isCancelled) {
        ...
        throttledUpdate({ subtitle: t('checkingYoutubeAccess'), description: t('tryingAlternateClients') });
...
async function handleDownloadCommand(prefilledUrl?: string): Promise<void> {
  ...
  const progressResult = await sigma.ui.withProgress(
    ...
    async (progress, token) => {
      let onCancelHandler: (() => Promise<void>) | null = null;
      ...
      const downloadResult = await runYtDlp(installResult.binaryPath, {
        ...
        outputDir,
        onProgress: (report: ProgressReport) => progress.report(report),
...
async function handleStartupActivation(): Promise<void> {
